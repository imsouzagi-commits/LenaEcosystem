...
from openjarvis.lena.desktop_controller import LenaDesktopController
from openjarvis.lena.fallback_controller import LenaFallbackController
from openjarvis.lena.file_controller import LenaFileController
...
from openjarvis.lena.search_orchestrator import LenaSearchOrchestrator
...
from openjarvis.lena.smart_brain import LenaSmartBrain
...
class LenaAgent:
    ...
    def __init__(self) -> None:
        self.kernel = LenaKernel()
        self.memory_engine = LenaMemoryFacade()
        self.desktop_controller = LenaDesktopController(self.kernel)
        self.file_controller = LenaFileController(self.kernel)
        self.fallback_controller = LenaFallbackController()
        self.smart_brain = LenaSmartBrain()
        self.cognitive = LenaCognitiveOrchestrator()
        self.searcher = LenaSearchOrchestrator()
    ...
    def _requires_kernel_ready(self, routes: set[str]) -> bool:
        return bool(routes & self.KERNEL_READY_ROUTES)
    ...
    def run(self, messages: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if self.memory_engine.needs_learning(user_text.lower().strip()):
        ...
        for command in split_commands:
            cognitive = self.cognitive.classify(command, self.memory_engine)
            route = UnifiedRouteResolver.resolve(cognitive)
            classified_commands.append((command, route, cognitive))

        planned_routes = {route for _, route, _ in classified_commands}
        self.last_route = "TASK_CHAIN" if len(classified_commands) > 1 else next(iter(planned_routes))
        self.last_route_used = self.last_route

        self.memory_engine.ingest_user_turn(user_text, self.last_route)

        if self._requires_kernel_ready(planned_routes) and not self.kernel.await_ready(timeout=5.0):
            self.last_latency_ms = round((time.perf_counter() - started) * 1000, 3)
            return self._fast_return("kernel Lena ainda está inicializando.")

        executed = []

        for command, route, cognitive in classified_commands:
            if route == "LOCAL_SEARCH":
                tool_payload = self.searcher.local_search(command)
                tool_payload = "não encontrei nada realmente útil no código local." if not tool_payload else tool_payload[0]
                response = self.conversationer.respond(self, command, cognitive=cognitive, tool_payload=tool_payload)

            elif route == "WEB_SEARCH_BG":
                tool_payload = self.searcher.web_search(command)
                response = self.conversationer.respond(self, command, cognitive=cognitive, tool_payload=tool_payload)

            else:
                response = LenaIntentDispatcher.dispatch(self, route, command, cognitive=cognitive)

            executed.append((route, response or "certo."))

        final_response = " | ".join(resp for _, resp in executed)
        executed_routes = {r for r, _ in executed}
        self.last_route_used = self.last_route if len(executed_routes) > 1 else next(iter(executed_routes))

        if not all(r in {"DESKTOP", "FILE_OP", "WEB_SEARCH_BG", "LOCAL_SEARCH"} for r in executed_routes):
            final_response = self.turn_manager.finalize_turn(user_text, final_response)
        else:
            self.memory_engine.push_exchange(
                user_text,
                final_response,
                light=True,
                route_used=self.last_route_used,
    ...
    def run_stream(self, messages: List[Dict[str, Any]]) -> Generator[str, None, None]:
        final = self.run(messages)["choices"][0]["message"]["content"]
        yield final
