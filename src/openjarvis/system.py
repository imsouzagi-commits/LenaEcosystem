...
SYSTEM_PROMPT = """
Você é a Lena, uma assistente natural e inteligente.

Regras:
- Não repita o que o usuário diz
...
class JarvisSystem:
    ...
    def montar_messages(self, user_input: str) -> List[Message]:
        base = [
    ...
    def _resposta_repetida(self, resposta: str) -> bool:
        """Check if response is too similar to recent history."""
        for msg in self.chat_history[-5:]:
            if msg.role == Role.ASSISTANT and resposta.strip().lower() == msg.content.strip().lower():
                return True
        return False

    def _ajustar_resposta(self, resposta: str, query: str) -> str:
        """Improve naturalness, avoid repetition, and add contextual proactivity."""
        lower = resposta.lower()
        q = query.lower().strip()

        # Evita respostas espelhadas tipo "e você?"
        if "e você" in lower and q in lower:
            return "tudo certo por aqui também 😄"

        # Evita duplicação simples de cumprimento
        if q in ["oi", "olá", "ola"]:
            return "e aí, tudo tranquilo?"

        # 🔥 NOVO: adiciona leve proatividade contextual
        # Se for conversa casual, adiciona continuação natural
        if any(word in q for word in ["tudo bem", "como você tá", "como vc tá"]):
            return resposta + " o que você anda aprontando hoje?"

        # Se usuário fizer pergunta simples, incentiva continuidade
        if q.endswith("?") and len(q.split()) < 6:
            return resposta + " quer ir mais a fundo nisso?"

        return resposta

    def _detectar_tipo(self, query: str) -> str:
        q = query.lower()
        if any(p in q for p in ["você está", "vc está", "isso está", "errado"]):
            return "critica"
        if "?" in q:
            return "pergunta"
        if any(p in q for p in ["oi", "olá", "ola", "e aí", "fala"]):
            return "conversa"
        return "geral"

    def _aplicar_estilo(self, resposta: str, query: str) -> str:
        tipo = self._detectar_tipo(query)

        # crítica → reconhecer e ajustar
        if tipo == "critica":
            return "boa, você tem razão — deixa eu ajustar isso. " + resposta

        # pergunta → convidar aprofundamento
        if tipo == "pergunta":
            return resposta + " quer que eu te mostre um exemplo prático?"

        # conversa → manter fluxo leve
        if tipo == "conversa":
            return resposta + " e você?"
    ...
    def ask(
        ...
        *,
        ...
        messages = self.montar_messages(query)
        ...
        content = self._ajustar_resposta(content, query)
        content = self._aplicar_estilo(content, query)

        # Anti-repetition: regenerate once if repeated
        if self._resposta_repetida(content):
            result = self.engine.generate(
                messages=messages,
                model=self.model,
                temperature=temperature,
        ...
        self.chat_history.append(Message(role=Role.USER, content=query))
        self.chat_history.append(Message(role=Role.ASSISTANT, content=content))
    ...
    def _run_agent(
        ...
        *,
        ...
        # Anti-repetition for agent
        if self._resposta_repetida(content):
            content = "deixa eu responder isso de outra forma: " + content
    ...
    def wire_channel(self, channel_bridge: Any) -> None:
        ...
        def _on_channel_message(cm) -> None:
            ...
            if _system.session_store is None:
                return
