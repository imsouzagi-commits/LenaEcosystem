"""Cloud inference engine — Azure OpenAI, Anthropic, Google, and MiniMax backends."""
...
# Silence optional import warnings for anthropic/google in type checking
try:
...
# Azure OpenAI defaults for this deployment
...
_AZURE_MODELS = [
    _AZURE_OPENAI_DEFAULT_DEPLOYMENT,
]
# Backward-compat alias used by existing tests and integrations.
_OPENAI_MODELS = _AZURE_MODELS
_ANTHROPIC_MODELS = [
...
class CloudEngine(InferenceEngine):
    """Cloud inference via Azure OpenAI, Anthropic, Google, and MiniMax SDKs."""
    ...
    # Backward-compat shim for existing tests/integrations that still set/read
    # `_openai_client`; internally we route everything through Azure.
    @property
    def _openai_client(self) -> Any:
        return self._azure_openai_client

    @_openai_client.setter
    def _openai_client(self, value: Any) -> None:
        self._azure_openai_client = value
    ...
    def _init_clients(self) -> None:
        ...
        if api_key:
            try:
                ...
                logger.warning(
                    "Azure OpenAI client unavailable: install openai package",
                )
        else:
            logger.debug("AZURE_OPENAI_API_KEY not set; Azure client disabled")
    ...
    def _generate_openai(
        ...
        **kwargs: Any,
        ...
        if self._azure_openai_client is None:
            raise EngineConnectionError(
                "Azure OpenAI client not available — set "
                "AZURE_OPENAI_API_KEY and install "
                "openjarvis[inference-cloud]"
            )
        deployment = model or _AZURE_OPENAI_DEFAULT_DEPLOYMENT
        ...
        if not _is_openai_reasoning_model(deployment):
        ...
        try:
            ...
            if status_code == 401:
                logger.error(
                    "Azure OpenAI auth failed (401). Check AZURE_OPENAI_API_KEY.",
                )
                raise EngineConnectionError(
            ...
            if status_code == 404:
                logger.error(
                    "Azure deployment '%s' not found at %s",
                    deployment,
                    _AZURE_OPENAI_ENDPOINT,
                )
                raise EngineConnectionError(
                    f"Azure deployment '{deployment}' not found (404). "
                    "Verify endpoint and deployment name.",
                ) from exc
            if "name resolution" in msg.lower() or "nodename nor servname" in msg.lower():
                logger.error(
                    "Azure endpoint resolution failed: %s",
                    _AZURE_OPENAI_ENDPOINT,
                )
                raise EngineConnectionError(
                    "Azure endpoint is invalid or unreachable: "
                    f"{_AZURE_OPENAI_ENDPOINT}",
                ) from exc
        ...
        content_val = getattr(choice.message, "content", "")
        result = {
    ...
    async def _stream_openai(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise EngineConnectionError(f"Azure OpenAI stream failed: {exc}") from exc
        for chunk in resp:
            delta = chunk.choices[0].delta if chunk.choices else None
            content_val = getattr(delta, "content", None) if delta else None
            if content_val:
                yield content_val
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if provider_name == "azure":
                ...
                if status_code == 404:
                    raise EngineConnectionError(
                        f"Azure deployment '{create_kwargs.get('model')}' not found (404).",
                    ) from exc
                raise EngineConnectionError(
                    f"Azure OpenAI streaming request failed: {exc}",
                ) from exc
            ...
            raise EngineConnectionError(
                f"MiniMax streaming request failed: {exc}",
            ) from exc
        for chunk in resp:
    ...
    def health(self) -> bool:
        return (
            self._azure_openai_client is not None
    ...
    def close(self) -> None:
        if self._azure_openai_client is not None:
            if hasattr(self._azure_openai_client, "close"):
                self._azure_openai_client.close()
