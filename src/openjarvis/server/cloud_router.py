...
_AZURE_OPENAI_DEPLOYMENT = "gpt-4.1"
...
async def _stream_azure_openai(
    ...
    if not api_key:
        raise ValueError(
            "AZURE_OPENAI_API_KEY not set — add it in the Cloud Models tab",
        )

    deployment = model or _AZURE_OPENAI_DEPLOYMENT
    if deployment != _AZURE_OPENAI_DEPLOYMENT:
    ...
    url = (
        f"{_AZURE_OPENAI_ENDPOINT.rstrip('/')}/openai/deployments/"
        f"{deployment}/chat/completions"
    )
    params = {"api-version": _AZURE_OPENAI_API_VERSION}
    headers = {
        "api-key": api_key,
        "Content-Type": "application/json",
    }
...
async def stream_cloud(
    ...
    if provider == "openai":
        async for token in _stream_azure_openai(
            model,
            messages,
            temperature,
            max_tokens,
