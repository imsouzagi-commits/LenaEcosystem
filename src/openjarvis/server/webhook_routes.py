            ...
            try:
                ...
                if twilio_client and twilio_from:
                    twilio_client.messages.create(
                        body=("Mensagem recebida! Estou trabalhando nisso agora..."),
                        from_=twilio_from,
                        to=from_number,
                    )
        ...
        def _handle_and_reply() -> None:
            ...
            if reply_channel:
                if position > 1:
                    reply_channel.send(
                        from_number,
                        f"Mensagem recebida! Mensagem {position} na fila, responderei em breve.",
                    )
                else:
                    reply_channel.send(
                        from_number,
                        "Mensagem recebida! Estou trabalhando nisso agora...",
                    )
