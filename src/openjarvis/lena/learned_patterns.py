...
class LenaLearnedPatterns:
    FILE_PATH = Path(__file__).resolve().parent / "learning_bank" / "patterns.json"
    ...
    def load(cls) -> Dict[str, List[str]]:
        ...
        try:
            ...
            if not isinstance(data, dict):
                raise ValueError("invalid learned patterns")
    ...
    @classmethod
    def add_markers(cls, topic: str, markers: List[str]) -> None:
        ...
        for marker in markers:
            cleaned = str(marker).strip().lower()
            if len(cleaned) >= 4:
                current.add(cleaned)
