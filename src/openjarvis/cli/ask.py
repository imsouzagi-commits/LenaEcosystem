...
from openjarvis.quick_commands import eh_comando_simples, executar_comando_simples, processar_comando
...
def ask(
    ...
    # Check if this is a simple command that can be executed quickly
    cmd_processado = processar_comando(query_text)
    if eh_comando_simples(cmd_processado):
        # Fast response for simple commands
        resposta_rapida = executar_comando_simples(cmd_processado)
        click.echo(resposta_rapida)
        return
